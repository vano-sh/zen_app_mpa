...
export const ModalSlider = () => {
  ...
  useEffect(() => {
    const debouncedHandleSliderCentering = debounce(handleSliderCentering, 500)

    window.addEventListener('resize', debouncedHandleSliderCentering)

    return () =>
      window.removeEventListener('resize', debouncedHandleSliderCentering)
  }, [])
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timerID)
  }, [isImageBoxBackward, dispatch, previewDetails, setPreviewDetails])
  ...
  useEffect(() => {
    ...
    sliderRef.current.style.top =
      screenHeight / 2 - sliderRef.current.offsetHeight / 2 + 'px'
    sliderRef.current.style.left =
  ...
  return (
    <div className={classes.modal_slider}>
      ...
      />
      <div className={classes.overlay} onClick={handleCloseClick}></div>
    </div>
  )
}
