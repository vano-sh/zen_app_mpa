import { useChatbot } from 'shared/model/hooks/useChatbot'
            ...
            <ol className={classes.list}>
              {message.map((item, index) => (
                <li key={index} className={classes.item}>
                  ...
                  <button onClick={handleQuestionClick}>{item.question}</button>
                </li>
              ))}
            </ol>

            <span className={classes.date}>{getCurrentTime()}</span>
          </div>
        )
